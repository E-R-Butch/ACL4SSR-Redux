...
PyPI 安装分两段：[包索引由 `pypi.org` 提供](https://docs.pypi.org/api/index-api/)，[wheel 文件由 `files.pythonhosted.org` 提供](https://docs.pypi.org/api/)。原先 `DeveloperDirect.list` 排在 `DeveloperProxy.list` 前，两个宽泛直连后缀先命中，导致开发代理表中的 PyPI 条目无效。现保留 `DeveloperCDN` 优先，将 `DeveloperProxy` 放到 `DeveloperDirect` 前，并在既有代理表中只匹配这两个精确主机；直连表的后缀规则仍处理其他 PyPI、pythonhosted 子域。实测 NumPy JSON 项目元数据 3.67 MB，直连 30 秒仅收约 239 KB，当前代理 2.41 秒完整返回；PyYAML 简易索引 290 KB，直连 25 秒差 6.6 KB 未完成，代理 1.43 秒完成；NumPy macOS arm64 wheel 的前 1 MiB 在本次固定目标 IP 上直连 TCP/443 超时，代理 2.10 秒返回有效 206 文件内容。样本支持索引和文件下载一并代理，不推断其他子域的速度。
